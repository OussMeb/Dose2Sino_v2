...
def plot_live_inference_result(result: dict[str, Any], max_dose: float, title: str) -> plt.Figure:
    ...
    def show(ax, img, name, cmap, vmin=None, vmax=None):
        im = ax.imshow(img, cmap=cmap, vmin=vmin, vmax=vmax, aspect="auto")
        ax.set_title(name)
        ax.axis("off")
        fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)

    show(axes[0, 0], baseline, "Frozen baseline", "hot", 0, 1)
    show(axes[0, 1], pred, "Prediction / refined", "hot", 0, 1)
    show(axes[0, 2], gt, "GT", "hot", 0, 1)

    show(axes[1, 0], np.abs(baseline - gt), "|Baseline-GT|", "RdYlGn_r", 0, 0.5)
    show(axes[1, 1], np.abs(pred - gt), "|Pred-GT|", "RdYlGn_r", 0, 0.5)
    show(axes[1, 2], dose, "Dose Berlingo mid CP", "hot", 0, max_dose)

    if delta is not None:
        vmax = max(float(np.max(np.abs(delta))), 1e-6)
        show(axes[2, 0], delta, "Predicted delta", "coolwarm", -vmax, vmax)
        show(axes[2, 1], gt - baseline, "True delta = GT-baseline", "coolwarm", -vmax, vmax)
        show(axes[2, 2], ct, "CT Berlingo mid CP", "gray", 0, 1)
