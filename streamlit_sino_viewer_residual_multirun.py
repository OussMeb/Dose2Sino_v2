...
def folder_label(folder: Path) -> str:
    ...
    if parent and parent not in {"runs", "visualizations"}:
        return f"{parent}/{folder.name}"
    return folder.name or str(folder)


def parse_run_roots(raw_paths: str, fallback_dir: Path) -> list[Path]:
    paths = [
        Path(line.strip()).expanduser()
        for line in raw_paths.splitlines()
        if line.strip() and not line.strip().startswith("#")
    ]
    return paths if paths else [fallback_dir]


def discover_loss_history_runs(raw_paths: str, fallback_dir: Path, recursive: bool) -> dict[str, pd.DataFrame]:
    roots = parse_run_roots(raw_paths, fallback_dir)
    ...
    for root in roots:
        ...
        if root.is_file() and root.name == "loss_history.csv":
            files.append(root)
        elif root.is_dir():
            direct = root / "loss_history.csv"
            if direct.exists():
                files.append(direct)

            if recursive:
                files.extend(sorted(root.rglob("loss_history.csv")))

        for loss_file in sorted(set(files)):
            ...
            df = df.copy()
            df.attrs["folder"] = str(run_dir)
            df.attrs["source"] = str(loss_file)
            runs[label] = df
...
def plot_loss_history(output_dir: Path) -> None:
    ...
    st.caption(
        "Paste one run folder, several run folders, or a parent root folder. "
        "The viewer searches for `loss_history.csv` files."
    )

    raw_paths = st.text_area(
        "Run folders / root folders, one per line",
        value=str(output_dir),
        height=120,
        key="curve_compare_roots",
    )

    recursive = st.checkbox(
        "Search recursively inside root folders",
        value=True,
        key="curve_compare_recursive",
    )

    runs = discover_loss_history_runs(raw_paths, output_dir, recursive=recursive)

    if not runs:
        st.info("No loss_history.csv files found.")
        return
    ...
    selected_runs = st.multiselect(
        ...
        key="curve_compare_selected_runs",
    )
    ...
    if not metric_options:
        st.info("No numeric metrics found in selected runs.")
        return
    ...
    if not plotted:
        st.warning(f"No selected run contains metric `{metric}`.")
        return
    ...
    st.markdown("#### Best epoch summary")
    ...
    for label in selected_runs:
        ...
        for extra in ["val_mae", "val_open_l1", "val_closed_abs_pred", "val_loss", "val_mean_abs_delta"]:
            if extra in df.columns:
                row[extra] = float(df.loc[idx, extra])
    ...
    with st.expander("Raw metric tables", expanded=False):
        for label in selected_runs:
            df = runs[label]
            if metric not in df.columns:
                continue
            st.write(f"### {label}")
            st.dataframe(df[["epoch", metric]].tail(100), use_container_width=True)
