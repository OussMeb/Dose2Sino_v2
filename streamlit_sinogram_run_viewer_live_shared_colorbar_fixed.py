...
def _add_shared_colorbar_live(fig, im, axes, label: str = "") -> None:
    cbar = fig.colorbar(im, ax=list(axes), fraction=0.055, pad=0.025)
    cbar.ax.tick_params(labelsize=8)
    if label:
...
def plot_live_inference_result(
    ...
    tall_height: float | None = None,
    sinogram_aspect: float = 0.55,
    sinogram_interpolation: str = "bilinear",
    panel_width: float = 2.15,
) -> list[plt.Figure]:
    """
    ...
    if display_mode == "Compact overview":
        ...
        im_sino = _imshow_with_colorbar_live(fig, axes[0, 0], baseline, "Frozen baseline", sino_cmap, 0, 1, "auto", interpolation=sinogram_interpolation, add_colorbar=False)
        _imshow_with_colorbar_live(fig, axes[0, 1], pred, "Prediction / refined", sino_cmap, 0, 1, "auto", interpolation=sinogram_interpolation, add_colorbar=False)
        _imshow_with_colorbar_live(fig, axes[0, 2], gt, "GT", sino_cmap, 0, 1, "auto", interpolation=sinogram_interpolation, add_colorbar=False)
        _add_shared_colorbar_live(fig, im_sino, [axes[0, 0], axes[0, 1], axes[0, 2]], label="Sinogram value")

        _imshow_with_colorbar_live(fig, axes[1, 0], abs_base_diff, "|Baseline - GT|", absdiff_cmap, 0, 0.5, "auto", interpolation=sinogram_interpolation)
        _imshow_with_colorbar_live(fig, axes[1, 1], abs_pred_diff, "|Pred - GT|", absdiff_cmap, 0, 0.5, "auto", interpolation=sinogram_interpolation)
        _imshow_with_colorbar_live(fig, axes[1, 2], signed_pred_diff, "Pred - GT", signed_cmap, -0.3, 0.3, "auto", interpolation=sinogram_interpolation)
        ...
        if delta is not None:
            ...
            vmax = min(vmax, 0.15)
            _imshow_with_colorbar_live(fig, axes[2, 0], delta, "Predicted delta", signed_cmap, -vmax, vmax, "auto", interpolation=sinogram_interpolation)
            _imshow_with_colorbar_live(fig, axes[2, 1], delta_target, "Target delta = GT - baseline", signed_cmap, -vmax, vmax, "auto", interpolation=sinogram_interpolation)
    ...
    n_cp, _ = gt.shape
    height = float(tall_height) if tall_height is not None else float(np.clip(n_cp / 70.0, 12.0, 28.0))
    aspect_value = float(sinogram_aspect)
    n_cols = 5 if delta is None else 6
    width = float(panel_width) * n_cols
    ...
    im_sino = _imshow_with_colorbar_live(fig, axes[0], baseline, "Baseline", sino_cmap, 0, 1, aspect_value, "Leaf", "Control point", sinogram_interpolation, add_colorbar=False)
    _imshow_with_colorbar_live(fig, axes[1], pred, "Prediction", sino_cmap, 0, 1, aspect_value, "Leaf", "", sinogram_interpolation, add_colorbar=False)
    _imshow_with_colorbar_live(fig, axes[2], gt, "Ground truth", sino_cmap, 0, 1, aspect_value, "Leaf", "", sinogram_interpolation, add_colorbar=False)
    _add_shared_colorbar_live(fig, im_sino, [axes[0], axes[1], axes[2]], label="Sinogram value")
    _imshow_with_colorbar_live(fig, axes[3], abs_pred_diff, "|Pred - GT|", absdiff_cmap, 0, 0.5, aspect_value, "Leaf", "", sinogram_interpolation)
    _imshow_with_colorbar_live(fig, axes[4], signed_pred_diff, "Pred - GT", signed_cmap, -0.3, 0.3, aspect_value, "Leaf", "", sinogram_interpolation)
    ...
    if delta is not None:
        ...
        vmax = min(vmax, 0.15)
        _imshow_with_colorbar_live(fig, axes[5], delta, "Predicted delta", signed_cmap, -vmax, vmax, aspect_value, "Leaf", "", sinogram_interpolation)
...
def tab_live_inference() -> None:
    ...
    run_clicked = st.button("Run inference on selected patient/pareto", type="primary")
    ...
    if run_clicked:
        try:
            ...
            st.session_state["live_inference_result"] = result
            st.session_state["live_inference_patient_id"] = str(patient_id)
            st.session_state["live_inference_pareto_index"] = str(pareto_index)
        except Exception as exc:
            st.error(f"Inference failed: {exc}")
            return

    if "live_inference_result" not in st.session_state:
        st.info("Run inference once. Then you can change colors, height, and aspect without recomputing.")
        return

    result = st.session_state["live_inference_result"]
    patient_id = st.session_state.get("live_inference_patient_id", str(patient_id))
    pareto_index = st.session_state.get("live_inference_pareto_index", str(pareto_index))

    metrics = sinogram_metrics_np(result["pred"], result["target"], result["baseline"])

    st.success(
        f"Inference loaded on {result['device']} | "
        f"checkpoint epoch={result['checkpoint_epoch']} | "
        f"refiner={result['has_refiner']} | "
        f"patient={patient_id} | pareto={pareto_index}"
    )
    ...
    with d2:
        sino_cmap = st.selectbox(
            "Sinogram colormap",
            ["hot", "inferno", "magma", "viridis", "turbo"],
            index=0,
            key="live_sino_cmap",
        )
    with d3:
        absdiff_cmap = st.selectbox(
            "Absolute diff colormap",
            ["magma", "inferno", "viridis", "turbo", "hot"],
            index=0,
            key="live_absdiff_cmap",
        )
    with d4:
        signed_cmap = st.selectbox(
            "Signed/delta colormap",
            ["coolwarm", "seismic", "bwr", "PiYG"],
            index=0,
            key="live_signed_cmap",
        )

    s1, s2, s3, s4 = st.columns(4)
    with s1:
        tall_height = st.slider(
            "Tall view height",
            min_value=6.0,
            max_value=30.0,
            value=14.0,
            step=1.0,
            help="Lower value makes the tall sinogram figure shorter.",
            key="live_tall_height",
        )
    with s2:
        sinogram_aspect = st.slider(
            "Vertical scale / pixel aspect",
            min_value=0.20,
            max_value=1.00,
            value=0.55,
            step=0.05,
            help="1.0 = true square pixels, 0.5 = shorter vertical display.",
            key="live_sino_aspect",
        )
    with s3:
        sinogram_interpolation = st.selectbox(
            "Sinogram rendering",
            ["bilinear", "bicubic", "hanning", "nearest"],
            index=0,
            help="nearest = raw discrete pixels. bilinear/bicubic = smoother display only.",
            key="live_sino_interpolation",
        )
    with s4:
        panel_width = st.slider(
            ...
            value=2.15,
            step=0.05,
            help="Higher value makes each sinogram panel wider. Use 2.0–2.5 to avoid tiny panels.",
            key="live_panel_width",
        )
    ...
    figures = plot_live_inference_result(
        ...
        tall_height=float(tall_height),
        sinogram_aspect=float(sinogram_aspect),
        sinogram_interpolation=str(sinogram_interpolation),
