...
# -*- coding: utf-8 -*-
"""
exp_base48.py  —  LEVER 2 (capacity), methodical A/B vs the geometry-fixed baseline.

ONLY variable changed vs main_2p5d.py: BASE_FILTERS 24 -> 48.
Everything else identical (same cache_sino_r8 / TARGET_HW=64, same loss, same
cosine LR, same flip-aug, same seed/splits). Tests the "model underfits"
hypothesis: at base24 train_open_l1 ~= val (~0.158) yet sanity reaches 0.03, so
there is unused capacity. If base48 lowers val open_l1 below 0.158, capacity is
a real lever; if not, the floor is data/inverse-problem, not capacity.

Baseline to beat: val open_l1 0.158 (base24, geometry-fixed).

    CUDA_VISIBLE_DEVICES=0 PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
      .venv/bin/python -u exp_base48.py
"""
...
if __name__ == "__main__":
    ...
    config.BATCH_SIZE = 1
    config.BASE_FILTERS = 48         # <-- LEVER 2: capacity 24 -> 48 (only change)
    ...
    config.JOURNAL = (
        ...
        "Only change vs baseline: BASE_FILTERS 24->48. Baseline val open_l1 0.158."
    )
    ...
    trainer = TrainerSupervisedLogits(
        ...
        loss_function=loss, phase_suffix="2p5d_base48",
    )
